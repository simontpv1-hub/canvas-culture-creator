...
const PopularCategories = () => {
  ...
  // Use collections after the first 10 (featured), or just show last 6
  const popular = collections.slice(10, 16).length >= 3
    ? collections.slice(10, 16)
    ...
    return (
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-20">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-display font-semibold text-foreground">Popular Categories</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="aspect-[4/3] bg-secondary animate-pulse rounded" />
          ))}
        </div>
      </section>
    );
